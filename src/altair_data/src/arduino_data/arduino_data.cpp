...
void Arduino_Data::process()
{
  Arduino_Data::initialize();
  if(reconnect) {
    close(fd);
    if(Arduino_Data::initialize()) {
      reconnect = false;
    }
  }
  std::ifstream serial(portName);

  while (getline(serial, read_buffer) && rclcpp::ok()) {
    ...
    if (iss >> type >> value) {
      ...
      reconnect = true;
      RCLCPP_ERROR(rclcpp::get_logger("arduino_data_node"), "Error Parsing Data. Trying to reconnect Now... \nIf the error persist, try to reconnect USB or restart arduino_data_node");
    }
  }
}
