...
Pose3D getPose3DfromTransform(const Eigen::Affine3d& transform)
{
  Pose3D pose_3d;
  ...
  pose_3d.x     = translation.x();
  pose_3d.y     = translation.y();
  pose_3d.z     = translation.z();
  pose_3d.roll  = atan2( rotation.coeff(2,1), rotation.coeff(2,2));
  pose_3d.pitch = atan2(-rotation.coeff(2,0), sqrt(rotation.coeff(2,1)*rotation.coeff(2,1) + rotation.coeff(2,2)*rotation.coeff(2,2)) );
  pose_3d.yaw   = atan2( rotation.coeff(1,0), rotation.coeff(0,0));
