...
namespace rc_utils
{
	...
	**/
	...
	inline tf2::Vector3 eigenToTF(const Eigen::Vector3d& vec)
	{
		// Return the required tf vector
		return tf2::Vector3(vec.x(), vec.y(), vec.z());
	}

	//! @brief Convert a 3x3 Eigen matrix into a 3x3 TF matrix
	inline tf2::Matrix3x3 eigenToTF(const Eigen::Matrix3d& mat)
	{
	...
	inline Eigen::Vector3d tfToEigen(const tf2::Vector3& vec)
	{
	...
	inline Eigen::Matrix3d tfToEigen(const tf2::Matrix3x3& mat)
	{
